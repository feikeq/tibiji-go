...
CREATE TABLE IF NOT EXISTS `sys_user` (
    ...
    `email` varchar(255) NOT NULL DEFAULT '' COMMENT '邮箱(也可做登录名)',
    `cell` varchar(255) NOT NULL DEFAULT '' COMMENT '电话(也可做登录名)',
    ...
    `userclan` varchar(255) NOT NULL DEFAULT '' COMMENT '用户拓谱图(以逗号分隔)',
    `fname` varchar(255) NOT NULL DEFAULT '' COMMENT '真实姓名',
    `bankcard` varchar(255) NOT NULL DEFAULT '' COMMENT '银行卡',
    `identity_card` varchar(255) NOT NULL DEFAULT '' COMMENT '身份证(也可做登录名)',
    `grouptag` varchar(255) NOT NULL DEFAULT '' COMMENT '用户组',
...
CREATE TABLE IF NOT EXISTS `sys_oauth` (
  ...
  `platfrom` varchar(255) NOT NULL DEFAULT '' COMMENT '外接平台名(微信、支付宝、AppID..)',
  `openid` varchar(255) BINARY NOT NULL DEFAULT '' COMMENT '外接平台身份ID(区分大小写)',
  `headimg` varchar(255) NOT NULL DEFAULT '' COMMENT '头像',
  `unionid` varchar(255) NOT NULL DEFAULT '' COMMENT '外接唯一标识[英文帐号]',
  `nickname` varchar(255) NOT NULL DEFAULT '' COMMENT '昵称',
  `sex` char(1) NOT NULL DEFAULT '0' COMMENT '性别(0未知 1男 2女)',
  `city` varchar(255) NOT NULL DEFAULT '' COMMENT '城市',
  `province` varchar(255) NOT NULL DEFAULT '' COMMENT '省份',
  `country` varchar(255) NOT NULL DEFAULT '' COMMENT '国家',
  `language` varchar(255) NOT NULL DEFAULT '' COMMENT '语言(zh_CN、EN)',
  `privilege` text COMMENT '用户特权信息(json数组)[关注数粉丝数等]',
  `token` varchar(255) DEFAULT '' COMMENT '平台接口授权凭证',
  `expires` varchar(255) NOT NULL DEFAULT '' COMMENT '平台授权失效时间',
  `refresh` varchar(255) NOT NULL DEFAULT '' COMMENT '平台刷新token',
  `scope` varchar(255) NOT NULL DEFAULT '' COMMENT '用户授权的作用域(使用逗号分隔)',
  `subscribe` int NOT NULL DEFAULT '0' COMMENT '是否关注[或是follow数]',
  `subscribetime` varchar(255) NOT NULL DEFAULT '' COMMENT '关注时间(时间戳)',
  `grouptag` varchar(255) NOT NULL DEFAULT '' COMMENT '分组',
...
CREATE TABLE IF NOT EXISTS `sys_material` (
  ...
  `tag` varchar(255) NOT NULL DEFAULT '' COMMENT '权限标识',
  `remark` varchar(255) NOT NULL DEFAULT '' COMMENT '备注',
  `object` text COMMENT '预留字段(不检索,可存json数组)',
  `intime` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '入库时间(自动)',
  `uptime` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间(自动)',
...
CREATE TABLE IF NOT EXISTS `sys_logs` (
  ...
  `action` varchar(255) NOT NULL DEFAULT '' COMMENT '操作标识(login,pay,sys)',
...
CREATE TABLE IF NOT EXISTS `tbj_contact` (
  ...
  `company` varchar(75) DEFAULT '' COMMENT '公司部门',
  `position` varchar(32) DEFAULT '' COMMENT '职位头衔',
  ...
  `family` varchar(128) DEFAULT '' COMMENT '家庭户主',
...
CREATE TABLE IF NOT EXISTS `tbj_account` (
  ...
  `note` varchar(50) DEFAULT '' COMMENT '备注说明',
