...
type HumaneGroup struct {
	...
	Last     int64   `json:"last" description:"最后往来时间"`
	Total    int     `json:"total" description:"往来总次数"`
	IncTotal int     `json:"inc_total" description:"回报总次数"`
	OutTotal int     `json:"out_total" description:"付出总次数"`
	// List     []HumaneItems `json:"list" description:"帐单列表"`
	IncList []HumaneItems `json:"inc_list" description:"回报帐单列表"`
	OutList []HumaneItems `json:"out_list" description:"付出帐单列表"`
}
...
// 人情往来列表
func (m *HumaneModel) Items(uid int64, pageOrder, pageField string, listSize int) ([]HumaneGroup, error) {
	/*
		SELECT a.`cid`,c.`fullname`,c.`picture`,c.`pinyin`,a.`item`, a.`class`, a.`sort`, a.`object`, a.`accounts`,a.`note`, a.`btime`, a.`money`, a.`uptime`
	...
	*/
	...
	query += fmt.Sprintf("WHERE a.`uid` = %d AND a.`cid` > 0 AND a.cid = c.cid ", uid)
	query += "ORDER BY a.`intime` DESC "

	// 拼接 GET 的 select 查询语句
	sql := "SELECT a.`cid`,c.`fullname`,c.`picture`,c.`pinyin`,a.`item`, a.`class`, a.`sort`, a.`object`, a.`accounts`,a.`note`, a.`btime`, a.`money`, a.`uptime` " + query
	// println("\r\n", sql) // 打印sql
	...
	for rows.Next() {
		...
		// 转换时间格式
		info.Btime = utils.RFC3339ToString(info.Btime, 0) // 脱敏 "2006-01-02"
		info.Uptime = utils.RFC3339ToString(info.Uptime)  // "2006-01-02 15:04:05"
		...
		if _, ok := list[key]; !ok {
			list[key] = &HumaneGroup{
				...
				IncList: make([]HumaneItems, 0),
				OutList: make([]HumaneItems, 0),
			}
		}
		tmp := HumaneItems{
			Item:     info.Item,
		...
		if list[key].Last < lastUnix {
			list[key].Last = lastUnix // 更新为最近的时间
		}
		...
		if info.Item == "收入" {
			...
			if len(list[key].IncList) < listSize {
				list[key].IncList = append(list[key].IncList, tmp)
			}
			// 添加往来总次数
			list[key].Total += 1
			list[key].IncTotal += 1
			...
			if len(list[key].OutList) < listSize {
				list[key].OutList = append(list[key].OutList, tmp)
			}
			// 添加往来总次数
			list[key].Total += 1
			list[key].OutTotal += 1
	...
	for _, v := range list {
		...
		if v.OutRatio < v.IncRatio {
			...
			v.Coef = utils.Round2(v.OutRatio * float64(v.Total))
		} else {
			// v.Coef = utils.Round2(v.IncRatio * float64(len(v.List)))
			v.Coef = utils.Round2(v.IncRatio * float64(v.Total))
		}
	...
	sort.Slice(groups, func(i, j int) bool {
		...
		} else { // last 往来时间 - 最后往来时间戳
			if pageOrder == "DESC" {
				return groups[i].Last > groups[j].Last
			} else {
				return groups[i].Last < groups[j].Last
			}
		}
...
func (m *HumaneModel) Read(uid, cid int64) (HumaneGroup, error) {
	...
	*/
	...
	for rows.Next() {
		...
		if infoGroup.Last < lastUnix {
			infoGroup.Last = lastUnix // 更新为最近的时间
		}
		...
		if info.Item == "收入" {
			...
			infoGroup.IncList = append(infoGroup.IncList, tmp)
			// 添加往来总次数
			infoGroup.Total += 1
			infoGroup.IncTotal += 1
		} else if info.Item == "支出" {
			infoGroup.Out = utils.Round2(infoGroup.Out + info.Money)
			infoGroup.OutList = append(infoGroup.OutList, tmp)
			// 添加往来总次数
			infoGroup.Total += 1
			infoGroup.OutTotal += 1
		}
	...
	if infoGroup.OutRatio < infoGroup.IncRatio {
		// infoGroup.Coef = utils.Round2(infoGroup.OutRatio * float64(len(infoGroup.List)))
		infoGroup.Coef = utils.Round2(infoGroup.OutRatio * float64(infoGroup.Total))
	} else {
		// infoGroup.Coef = utils.Round2(infoGroup.IncRatio * float64(len(infoGroup.List)))
		infoGroup.Coef = utils.Round2(infoGroup.IncRatio * float64(infoGroup.Total))
	}
...
func (m *HumaneModel) ReportRatio(uid int64) (map[string]interface{}, error) {
	/*
		...
		如果一条记录都没有会显示 null 会报错 sql: Scan error on column index 0, name "inc": converting NULL to float64 is unsupported
		Models.ReportRatio Error:  sql: Scan error on column index 0, name "inc": converting NULL to float64 is unsupported
		*除非把结构体 Inc float64  改为 *Inc float64
		*使用 COALESCE 函数为NULL提供一个默认值。

		SELECT
		COALESCE(SUM(CASE WHEN `item` = '收入' THEN `money` ELSE 0 END),0) 'inc',
		COALESCE(SUM(CASE WHEN `item` = '支出' THEN `money` ELSE 0 END),0) 'out',
		COALESCE(SUM(CASE WHEN `item` NOT IN ('收入', '支出') THEN `money` ELSE 0 END),0) `oth`
		FROM `tbj_account`
		WHERE `uid` = 8 AND `cid` != 0
	*/
