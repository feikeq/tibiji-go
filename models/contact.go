...
func NewContactModel(db *sqlx.DB, cfg map[string]interface{}) *ContactModel {
	return &ContactModel{
		DB:        db,
...
func (m *ContactModel) Create(data map[string]interface{}) (int64, error) {
	...
	data["cid"] = utils.GenerateTimerID(99) // （13位时间戳+2随机尾数）随机数每位最大到9
	...
	// 判断是否存在字段 "picture"
	if _, ok := data["picture"]; ok {
		_pic := data["picture"].(string)
		if _pic != "" {
			// 从完整的文件 URL 中提取目录路径
			data["picture"] = utils.GetDirectoryPath(m.Cfg["UPLOAD_DOMAIN"].(string), _pic, m.Cfg["NO_PARAMETER"].(bool))
		}
	}
...
func (m *ContactModel) Update(uid, id int64, data map[string]interface{}) (int64, error) {
	...
	// println("picture =>", data["picture"].(string))
...
func (m *ContactModel) Read(id int64) (ContactInfo, error) {
	...
	fields := "`cid`,`uid`,`fullname`,`pinyin`,`nickname`,`picture`,`phone`,`mail`,`im`,`http`,`company`,`position`,`address`,`gender`,`birthday`,`lunar`,`grouptag`,`remind`,`relation`,`family`,`note`,`state`,`intime`,`uptime`"
	...
	info.Picture = utils.GetFullURL(m.Cfg["UPLOAD_DOMAIN"].(string), info.Picture) // 生成完整的文件 URL
...
func (m *ContactModel) List(uid int64, filters map[string]interface{}, pageNumber, pageSize int64, pageOrder, pageField, searchOR string) ([]ContactInfo, int64, error) {
	// 按结构体映射提交字段
	...
	/*
		SELECT `cid`,`uid`,`fullname`,`pinyin`,`nickname`,`picture`,`phone`,`mail`,`im`,`http`,`company`,`position`,`address`,`gender`,`birthday`,`lunar`,`grouptag`,`remind`,`relation`,`family`,`note`,`state`,`intime`,`uptime`
		FROM `tbj_contact`
		WHERE `uid` = 8 AND state = 1 AND ( fullname LIKE'%2%' OR nickname LIKE '%2%' )
		ORDER BY `cid` DESC LIMIT 0,18
	*/
	// 拼接 NamedQuery 的 select 查询语句
	query := fmt.Sprintf("FROM `%s` WHERE `uid` = %d AND state != 0 %s ", m.TableName, uid, strings.Join(where_arr, " "))
	if searchOR != "" {
		// 如果有模糊查询
		query += "AND ( `fullname` LIKE :searchOR OR `phone` LIKE :searchOR OR `pinyin` LIKE :searchOR ) "
