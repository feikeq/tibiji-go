...
// 创建一个新的模型实例
func NewUserModel(db *sqlx.DB, cfg map[string]interface{}) *UserModel {
	return &UserModel{
		DB:                db,
		TableName:         "sys_user",
...
type UserUnion struct {
	...
	Manage         int     `db:"manage" json:"manage" description:"管理权限"`
	Tag            string  `db:"tag" json:"tag" description:"权限标识"`
	MaterialRemark string  `db:"material_remark" json:"material_remark" description:"备注"`
	MaterialObject string  `db:"material_object" json:"material_object" description:"预留字段"`
	MaterialUptime string  `db:"material_uptime" json:"material_uptime" description:"更新时间"`
}
...
func (m *UserModel) Create(data map[string]interface{}) (int64, error) {
	...
	if _, ok := data["username"]; ok {
		if data["username"] != "" {
			if m.Check("username", data["username"].(string), 0) {
	...
	if _, ok := data["cell"]; ok {
		if data["cell"] != "" {
			if m.Check("cell", data["cell"].(string), 0) {
	...
	if _, ok := data["email"]; ok {
		if data["email"] != "" {
			if m.Check("email", data["email"].(string), 0) {
	...
	if _, ok := data["identity_card"]; ok {
		if data["identity_card"] != "" {
			if m.Check("identity_card", data["identity_card"].(string), 0) {
	...
	// 判断是否存在字段 "headimg"
	if _, ok := data["headimg"]; ok {
		_pic := data["headimg"].(string)
		if _pic != "" {
			// 从完整的文件 URL 中提取目录路径
			data["headimg"] = utils.GetDirectoryPath(m.Cfg["UPLOAD_DOMAIN"].(string), _pic, m.Cfg["NO_PARAMETER"].(bool))
		}
	}
...
func (m *UserModel) Update(id int64, data map[string]interface{}) (int64, error) {
	...
	if _, ok := data["cell"]; ok {
		if data["cell"] != "" {
			if m.Check("cell", data["cell"].(string), id) {
				return 0, fmt.Errorf("用户cell电话已存在")
			}
		}
	}
	...
	if _, ok := data["email"]; ok {
		if data["email"] != "" {
			if m.Check("email", data["email"].(string), id) {
				return 0, fmt.Errorf("用户email邮箱已存在")
			}
		}
	}
	...
	if _, ok := data["identity_card"]; ok {
		if data["identity_card"] != "" {
			if m.Check("identity_card", data["identity_card"].(string), id) {
				return 0, fmt.Errorf("用户identity_card身份证已存在")
			}
		}
	}
	...
	// println("headimg =>", data["headimg"].(string))
...
func (m *UserModel) Read(id int64) (UserInfo, error) {
	...
	*user.Headimg = utils.GetFullURL(m.Cfg["UPLOAD_DOMAIN"].(string), *user.Headimg) // 生成完整的文件 URL
...
func (m *UserModel) Check(fieldName, fieldValue string, id int64) bool {
...
// 获取用户列表
func (m *UserModel) List(filters map[string]interface{}, pageNumber, pageSize int64, pageOrder, pageField, searchOR string) ([]UserUnion, int64, error) {
	// 按结构体映射提交字段
	filters = utils.StructAssigMap(UserUnion{}, filters)
	...
	/*
		...
		SELECT
		  u.`uid`, u.`username`, u.`email`, u.`cell`, u.`nickname`, u.`headimg`, u.`sex`, u.`birthday`, u.`company`, u.`address`, u.`city`, u.`province`, u.`country`, u.`grouptag`, u.`state`, u.`intime`, u.`uptime`, u.`ciphers`, u.`regip`, u.`referer`, u.`inviter`, u.`userclan`, u.`fname`, u.`bankcard`, u.`identity_card`, u.`remark`, u.`object`
		  ,m.`cid`, m.`remark` 'material_remark', m.`object` 'material_object', m.`uptime` 'material_uptime', m.`balance`, m.`vip`, m.`exptime`, m.`manage`, m.`tag`
		FROM `sys_user` u, `sys_material` m
		WHERE u.`uid` = m.`uid`
		ORDER BY u.`uid` ASC
		LIMIT 0, 20
	*/
	...
	query := fmt.Sprintf("FROM `%s` u, `%s` m WHERE u.`uid` = m.`uid` %s ", m.TableName, m.MaterialTableName, strings.Join(where_arr, " "))
	if searchOR != "" {
		// 如果有模糊查询
		query += "AND ( u.`uid` LIKE :searchOR OR `username` LIKE :searchOR OR `email` LIKE :searchOR OR `cell` LIKE :searchOR OR `nickname` LIKE :searchOR OR `fname` LIKE :searchOR OR `regip` LIKE :searchOR ) "
		args["searchOR"] = "%" + searchOR + "%" // 找查uid、username、email、cell、nickname、fname、regip
		// fmt.Printf("Type: %T , Data: %v\n", args, args) // 打印 args 映射的内容
	}
	orderANDlimit := fmt.Sprintf("%s LIMIT %d,%d", order, starNum, endNum)
	// 拼接 GET 的 select 查询语句
	sql := "SELECT u.`uid`, u.`username`, u.`email`, u.`cell`, u.`nickname`, u.`headimg`, u.`sex`, u.`birthday`, u.`company`, u.`address`, u.`city`, u.`province`, u.`country`, u.`grouptag`, u.`state`, u.`intime`, u.`uptime`, u.`ciphers`, u.`regip`, u.`referer`, u.`inviter`, u.`userclan`, u.`fname`, u.`bankcard`, u.`identity_card`, u.`remark`, u.`object` "
	sql += ",m.`cid`, m.`remark` 'material_remark', m.`object` 'material_object', m.`uptime` 'material_uptime', m.`balance`, m.`vip`, m.`exptime`, m.`manage`, m.`tag` " + query + orderANDlimit
	...
	for rows.Next() {
		...
		*info.Headimg = utils.GetFullURL(m.Cfg["UPLOAD_DOMAIN"].(string), *info.Headimg) // 生成完整的文件 URL

		info.Exptime = utils.RFC3339ToString(info.Exptime)
		info.MaterialUptime = utils.RFC3339ToString(info.MaterialUptime)
...
func (m *UserModel) ReadMaterial(uid int64) (UserUnion, error) {
	// SQL注入问题：我们任何时候都不应该自己拼接SQL语句！

	// 拼接 GET 的 select 查询语句
	fields := "u.`uid`, u.`username`, u.`email`, u.`cell`, u.`nickname`, u.`headimg`, u.`sex`, u.`birthday`, u.`company`, u.`address`, u.`city`, u.`province`, u.`country`, u.`grouptag`, u.`state`, u.`intime`, u.`uptime`, u.`ciphers`, u.`regip`, u.`referer`, u.`inviter`, u.`userclan`, u.`fname`, u.`bankcard`, u.`identity_card`, u.`remark`, u.`object` "
	fields += ",m.`cid`, m.`remark` 'material_remark', m.`object` 'material_object', m.`uptime` 'material_uptime', m.`balance`, m.`vip`, m.`exptime`, m.`manage`, m.`tag` "
	from := fmt.Sprintf("FROM `%s` u, `%s` m WHERE u.`uid`=? AND u.`uid` = m.`uid` LIMIT 1 ", m.TableName, m.MaterialTableName)
	sql := "SELECT " + fields + from
	// println("\r\n", sql) // 打印sql

	var material UserUnion
	...
	*material.Headimg = utils.GetFullURL(m.Cfg["UPLOAD_DOMAIN"].(string), *material.Headimg) // 生成完整的文件 URL
...
func (m *UserModel) CreateOAuth(data map[string]interface{}) (UserOAuth, error) {
	...
	if _, ok := data["platfrom"]; !ok {
		return oUser, fmt.Errorf("platfrom外接平台名不能为空")
	} else {
	...
	if _, ok := data["openid"]; !ok {
		return oUser, fmt.Errorf("openid外接平台身份ID不能为空")
	} else {
	...
	oUser, err := m.FindOAuthOpenid(platfrom, openid)
	if err == nil {
		// 更新用户资料

		delete(data, "uid") // 删除 绑定帐号字段

		// 调取模型 - 根据ID更新数据库中的信息
		_, err := m.UpdateOAuth(oUser.OID, data)
		if err != nil {
	...
	// 如果找到 unionid 相符的用户
	oUser, err = m.FindOAuthUnionid(platfrom, unionid)
	if err == nil {
		// 绑定同一用户id并入库
		data["uid"] = oUser.UID
	}
	...
	data["oid"] = utils.GenerateTimerID() // （默认13位时间戳+3随机尾数）随机数每位最大到9
...
func (m *UserModel) FindOAuthOpenid(platfrom, openid string) (UserOAuth, error) {
	...
	// println("\r\n", sql) // 打印sql
	// println("查找某一平台的openid用户", platfrom, openid)

	err := m.DB.Get(&oUser, sql, openid, platfrom) // 查询单行数据，也可以用 NamedQuery
	if err != nil {
		println("Err: ", err.Error())
		return oUser, err
	}

	oUser.Intime = utils.RFC3339ToString(oUser.Intime, 2)
	oUser.Uptime = utils.RFC3339ToString(oUser.Uptime, 2)
	return oUser, nil // 返回结果
}

// 查找已绑定的所有平台
func (m *UserModel) FindOAuth(id int64) ([]UserOAuth, error) {

	var oUser []UserOAuth

	// 拼接 GET 的 select 查询语句
	fields := "`oid`,`uid`,`platfrom`,`openid`,`unionid`,`nickname`,`headimg`,`city`,`province`,`country`,`grouptag`,`language`,`intime`,`uptime`,`privilege`,`token`,`expires`,`refresh`,`scope`,`subscribe`,`subscribetime`,`tidings`,`remark`,`object`"
	sql := fmt.Sprintf("SELECT %s FROM `%s` WHERE `uid`=? ", fields, m.OauthTableName)
	// println("\r\n", sql) // 打印sql

	err := m.DB.Select(&oUser, sql, id) // 查询多行数据
	if err != nil {
		println("Err: ", err.Error())
		return oUser, err
	}
...
func (m *UserModel) FindOAuthUnionid(platfrom, unionid string) (UserOAuth, error) {
	...
	// println("\r\n", sql) // 打印sql
	// println("查找平台unionid用户", platfrom, unionid)

	err := m.DB.Get(&oUser, sql, unionid, platfrom) // 查询单行数据，也可以用 NamedQuery
	if err != nil {
		println("Err: ", err.Error())
		return oUser, err
	}
...
func (m *UserModel) FindOAuthOid(oid int64) (UserOAuth, error) {
	...
	err := m.DB.Get(&oUser, sql, oid) // 查询单行数据，也可以用 NamedQuery
	if err != nil {
		println("Err: ", err.Error())
		return oUser, err
	}
...
// 分组
func (m *UserModel) Groups() ([]KVObject, error) {
	sql := fmt.Sprintf("SELECT `grouptag` 'str',`grouptag` 'val' FROM `%s` ", m.TableName)
	sql += "WHERE `grouptag` !='' GROUP BY `grouptag`"
	// println(sql)
	var data []KVObject
	err := m.DB.Select(&data, sql) // 查询多行数据
	if err != nil {
		println("数据库select查询失败，", err.Error())
		return data, err
	}
