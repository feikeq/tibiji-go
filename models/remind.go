...
type RemindInfo struct {
	...
	State    int    `db:"state" json:"state" description:"状态类别(0删除 1正常 2纪念日 3闹铃)"`

	// 数据加工处理生成的附加字段
	RemindNum      int    `json:"remind_num" description:"剩余天数"`
	RemindDate     string `json:"remind_date" description:"生日的日期 YYYY-MM-DD"`
	RemindCNDate   string `json:"remind_cndate" description:"生日的日期 X月X日"` // 跨年后会是 X月X日 XXXX年
	...
	RemindDay      string `json:"remind_day" description:"今天明天后天N天后"` // 前天,昨天,今天,明天,后天
...
type RemindInfoTask struct {
	...
	// 用户数据
	// UserUid      int64   `db:"user_uid" json:"user_uid" description:"用户ID"`
		...
		if len(arr_type) > 0 {
			...
			for _, value := range arr_type {
				if value != "" {
					...
					// fmt.Println(">>>>>>", arr_item, len(arr_item)) // [] 1
					//原始字符串"value"为空字符串，因此使用strings.Split方法将其拆分后返回的数组arr_item只包含一个空字符串元素。
					//因此，arr_item的长度为1，即打印出的结果是 [""], 1。因为即使原始字符串是空的，拆分后的数组长度仍然是1。
					if len(arr_item) > 0 {
...
func (m *RemindModel) Items(uid int64, filters map[string]interface{}) ([]RemindInfo, error) {
	...
	// 对切片进行排序 - 根据 剩余天数 字段从小到大正序排列infos切片
...
func (m *RemindModel) Task() (RemindQueues, error) {
	...
	// // ps:同时只取100天内的数据 - 当前日期加99天
	// afterTenDays := now.AddDate(0, 0, 99) // 添加99天，往后百天
	// timeEndStr := fmt.Sprintf("%02d-%02d", int(afterTenDays.Month()), afterTenDays.Day())
	// // println("队列区间 99 + 1当天 = 100 ：", timeStartStr, timeEndStr)
	...
	/*
		...
		// 月份跨年就没意义了
		AND DATE_FORMAT(c.`birthday`,'%m-%d') >= '12-21' //没必要
	*/

	// 拼接 NamedQuery 的 select 查询语句 (注意 0000-00-00 00:00:00 在sqlx库中的NamedQuery函数不支持冒号（:）作为命名参数的一部分，因为它会将其解释为命名参数的分隔符。解决方法是使用问号（?）作为命名参数的占位符，然后将命名参数作为第二个参数传递给函数。)
	query := fmt.Sprintf("FROM `%s` c, `%s` u, `%s` m ", m.TableName, m.UserTableName, m.MaterialTableName)
	query += "WHERE c.`state` != 0 AND c.`birthday` > '0000-00-00' " // 取所有设置了生日的
	// query += "AND DATE_FORMAT(c.`birthday`,'%m-%d') >= ? AND DATE_FORMAT(c.`birthday`,'%m-%d') < ? " // 月份跨年就没意义了
	...
	for rows.Next() {
		...
		// println(info.UID, info.Birthday, info.Lunar)
		...
		// 只有小于99天的通知才有意义
		if info.RemindNum < 99 {
			// 数据处理 - 转换时间格式 脱敏
			info.Birthday = utils.RFC3339ToString(info.Birthday, 2)
			info.UserExptime = utils.RFC3339ToString(info.Birthday, 2)

			// fmt.Printf("Type: %T , Data: %v\n", info.Remind, info.Remind)

			// 清理一次性闹铃记录
			if info.State == 3 && info.RemindNum == 0 {
				// println(info.Cid, info.RemindNum, info.Birthday, info.UserExptime, info.State)
				num, _ := m.Delete(info.Cid)
				println(info.Cid, "清理一次性闹铃记录条数：", num)
			}
			...
			// Notice 消息推送队列
			if utils.InArray(info.RemindType.Notice, info.RemindNum) {
				// println("Notice:")
				// println(info.RemindNum, info.Birthday)
				// println(info.Cid, "|", info.UID, "|", info.Fullname)
			...
			// Message 聊天消息队列
			if utils.InArray(info.RemindType.Message, info.RemindNum) {
				// println("Message:")
				// println(info.RemindNum, info.Birthday)
				// println(info.Cid, "|", info.UID, "|", info.Fullname)
...
// 删除帐单 (物理删除)
func (m *RemindModel) Delete(id int64) (int64, error) {
	// 物理删除  delete 删除 (只删除一次性闹钟)
	sql := fmt.Sprintf("DELETE FROM `%s` WHERE `cid` = ? AND `state`=3", m.TableName)
