...
func main() {
	...
	*/
	...
	// 两个{}只是把相同路由组的放在一个区块，没有其他用特殊含义
	{
		...
		userController := controllers.NewUserController(db, otherCfg)
		oauthController := controllers.NewOauthController(db, otherCfg)
		accountController := controllers.NewAccountController(db)
		contactController := controllers.NewContactController(db, otherCfg)
		remindController := controllers.NewRemindController(db, otherCfg)
		humaneController := controllers.NewHumaneController(db)
		notepadController := controllers.NewNotepadController(db, otherCfg)
