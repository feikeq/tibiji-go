...
func SendSMS(ctx iris.Context, phone, templateId string, templateArgs []string) error {
	// utils.SendSMS(ctx, phone, smsTemplate[1], []string{"今天", "张惠妹", "22"})
	println("SendSMS:", phone, templateId)
	fmt.Printf("args: %+v\n", templateArgs) // 打印
	// 获取配置项
	otherCfg := ctx.Application().ConfigurationReadOnly().GetOther()
	...
	/*
		...
		{"Response":{"SendStatusSet":[{"SerialNo":"","PhoneNumber":"15801606878","Fee":0,"SessionContext":"","Code":"InvalidParameterValue.TemplateParameterLengthLimit","Message":"single template variable exceeds the specified limit of characters","IsoCode":""}],"RequestId":"e75b6671-4a0b-4697-b713-5f6f7574bc3d"}}
		utils.SendSMS(ctx, phone, smsTemplate[1], []string{"今天", "张惠妹", "22"})
		您好，目前短信变量长度不能超过6个字符，这里您这三个变量是挨着的，所以他们的长度限制是一起的，总长度不能超过6个字符哈，请您知悉
		您好，这个上边有说哈，短信变量如果是相邻的话，长度是一起限制的，不能超过6个字符哈
